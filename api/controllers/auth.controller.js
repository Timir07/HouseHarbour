...
export const google = async (req, res, next) => {
  try {
    ...
    if (user) {
      ...
      //creating user in mongodb so we generatePass as googleAuth doesn't use pass and aur field is true so we generare our own pass
      const generatedPassword =
        Math.random().toString(36).slice(-8) +
        Math.random().toString(36).slice(-8); //give decimal string of 36 char - 26,0-9; two times; sliced from end
